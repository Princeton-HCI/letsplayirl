    ...
    $.fn.pagepiling = function (custom) {
        ...
        function scrollPage(destination, animated) {
            var v ={
                ...
                activeIndex: $('.pp-section.active').index('.pp-section'),
                toMove: destination,
            ...
            if(typeof v.anchorLink !== 'undefined'){
                ...
                v.sectionsMoved = getSectionsMoved(v);
         ...
         */
        function performMovementDown(v){
            if(options.css3){
                ...
                setTimeout(function(){
                    v.destination.find("video").get(0).play();
                    v.animateSection.addClass('ease-fast');
                    transformContainer(v.animateSection, 'translate3d(0px, ' + yAxis + 'vh, -' + zAxis + 'px) scaleX(' + scale + ')', v.animated);
                    v.animateSection.attr('data-order', zAxis);
                    v.sectionsToMove.each(function(){
        ...
        function performMovementUp(v){
            if(options.css3){
                var index = v.activeIndex - v.sectionIndex;
                var indexMoved = v.sectionsMoved.length;
                const yAxis = 0;
                const zAxis = 0;
                const scale = 1;
        ...
        function getSectionsToMove(v){
            ...
            if(v.yMovement === 'down'){
                sectionToMove = $('.pp-section').map(function(index){
                    if (index < v.sectionIndex && index > v.activeIndex){
                        return $(this);
                    }
                });
            }else{
        ...
        function getSectionsMoved(v){
            var sectionMoved2;

            if(v.yMovement === 'down'){
                sectionMoved2 = $('.pp-section').map(function(index){
                    if (index >= v.sectionIndex || index < v.activeIndex){
                        return $(this);
                    }
                });
                var sectionMoved = sectionMoved2.sort(function(a, b) {
        ...
        */
        ...
        function getTranslate3d(){
            if (options.direction !== 'vertical') {
                return 'translate3d(-100%, -1.6%, 0px)';
            }

            return 'translate3d(0px, -150%, 0px) scaleX(1)';
        }
